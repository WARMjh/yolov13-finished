...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, WIoU=False, eps=1e-7):
    ...
    # IoU
    iou = inter / union
    if CIoU or DIoU or GIoU or WIoU:  # <--- 1. 这里加上 or WIoU
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if CIoU or DIoU or WIoU:  # <--- 2. 这里加上 or WIoU
            ...
            # === 3. 核心：在此处注入 WIoU 的距离惩罚项计算 ===
            elif WIoU: 
                exp_factor = torch.exp((rho2 / c2) ** 0.5)
                return iou, exp_factor  # 注意：WIoU 必须返回两个参数，供 loss.py 计算离群度
            # ===============================================
